...
public sealed class CaptionService : ICaptionService
{
    ...
    public async Task<Caption> GenerateAsync(SceneDna sceneDna, SceneNarrative narrative)
    {
        ...
        // YouTube titles come from a separate pool, data/captions/titles/{sceneType}.txt,
        // with the same base.txt fallback. They are one line each and far shorter than a
        // description, so they only ever carry the year placeholders.
        string rawTitles;
        try
        {

...
src/LifeOverYears/
  ...
  Services/PromptSmokeTest.cs C1–C84 checks over generated prompts
...
dotnet run -- --smoke-prompts    # C1–C84 + folder checks; rewrites output/smoke/**
...
- Every prompt behaviour worth keeping gets a numbered check in
  `PromptSmokeTest` (C1…C84, sequential). Add one when you change what prompts
  say; update the hard-coded expected strings when you change wording or numbers.
  Numbers are never reused: the brand-series work was specified against C59-C66,
  which were already taken, and landed at C75-C84 instead.
